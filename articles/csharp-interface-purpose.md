...
### 1. ポリモーフィズムによって実装ごとの条件分岐を利用側からなくせる
...
ここでは、注文処理を担うクラスを`CheckoutService`とします。支払い方法ごとに具体的な処理内容は異なりますが、注文を受け付ける`CheckoutService`から見れば、いずれも注文金額を支払うための処理です。クレジットカードで支払うか、銀行振込で支払うかまで、`CheckoutService`が意識する必要は本来ありません。
...
また、支払い方法に応じて処理を分ける必要があるのは、`Checkout`メソッドだけとは限りません。アプリケーション内の別の処理でも支払い方法による処理分けが必要になれば、そこにも同じような条件分岐が書かれます。利用側が支払い方法を判定する今の形では、アプリケーションのあちこちに同じような条件分岐が増えていきます。その場合、支払い方法を追加するたびにすべての分岐箇所を確認して変更しなければならず、保守しづらくなります。

**インターフェースを利用する場合**

そこで、支払い方法に共通する契約を`IPaymentMethod`として定めます。この例では、`IPaymentMethod`に支払うためのメソッド`Pay`だけを定めます。
...
`CheckoutService`は、実際の支払い方法がクレジットカードなのか銀行振込なのかを判定せず、`Pay()`を呼び出すだけです。コンストラクターに`CreditCardPaymentMethod`を渡せばクレジットカードによる支払い処理が、`BankTransferPaymentMethod`を渡せば銀行振込による支払い処理が実行されます。このように、同じメソッドを呼び出しても、そのメソッドを呼び出すオブジェクトの種類によって異なる動作をする仕組みをポリモーフィズムと呼びます。
...
もちろん、アプリケーション内のどこかでは、購入者が画面で選んだ支払い方法に応じて、使用する`IPaymentMethod`の実装を決める処理が必要です。ただし、その処理を`CheckoutService`の内部に置く必要はありません。インターフェースを利用しない例では、`Checkout()`が`PaymentType`を受け取り、支払い方法の選択と注文処理の両方を担っていました。インターフェースを利用する例では、購入者が選んだ支払い方法に対応する`IPaymentMethod`のオブジェクトを、`CheckoutService`のコンストラクターへ渡します。そのため、`CheckoutService`自身が支払い方法を選ぶ必要はなく、注文処理だけに集中できます。
