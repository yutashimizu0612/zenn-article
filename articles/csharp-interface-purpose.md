...
インターフェースのイメージや全体像を掴めるよう、身近な例とコードを交えながら、その仕組みと利用によって得られる効果を説明していきます。
...
つまり、インターフェースは「何ができるか」を表し、それを実装するクラスは「どのように実現するか」を表すということになります。
...
ただし、インターフェースが保証するのは、`Send()`が実装されることまでです。メールが正しく送信されるかといった処理内容までは保証しません。
...
:::message
ここから紹介するメリットの多くは、インターフェースだけに固有のものではありません。利用側が抽象クラスに依存する場合にも、同様の効果を得られます。抽象クラスとインターフェースのどちらが適切かは設計によって異なりますが、この記事ではその使い分けには踏み込みません。

また、この記事のサンプルは、インターフェースとそれを実装する具象クラスを同一のアプリケーション内で管理し、同時に変更できる状況を想定しています。外部の開発者が実装する公開ライブラリでは、インターフェースの変更が既存の実装を壊す可能性があるため、APIの互換性という別の観点が必要になります。この記事では、公開ライブラリの設計については扱いません。
:::
...
1〜5で説明した効果は抽象クラスでも得られます。一方、ここから説明する「一つのクラスに複数の役割を持たせ、それぞれの役割の型として扱う」使い方は、抽象クラスだけでは実現できず、インターフェースが必要です。

C#では、一つのクラスが複数のクラスを継承することはできません。一方、一つのクラスが複数のインターフェースを実装することは可能です。

そのため、C#では一つのクラスに複数の役割を持たせたいとき、それぞれの役割を表すインターフェースを組み合わせて実装します。
...
ただし、ここで紹介したメリットの多くは、利用側が抽象クラスに依存する場合にも得られます。インターフェースと抽象クラスのどちらを選ぶかは別の設計判断です。そのため、この記事の例を正解としてそのまま採用するのではなく、インターフェースを利用した場合の仕組みと効果を確認するための例としてご覧ください。

また、必要のないところにまでインターフェースを適用すると、ソースコードが読みづらくなりますし、理解するための学習コストも増えます。どのようなメリットを得たいのかを意識し、必要なところで使うことが大切です。

この記事が、インターフェースのイメージを掴む手助けになれば幸いです。

私自身、理解が十分でない点もあるかと思いますので、誤りや説明が不十分な点があれば、コメントでご指摘いただけるとありがたいです。
